...
`Result!` `Number of Support Vectors`는 결정경계와 가까이 위치한 case의 수이다. 해당 데이터에서는 총 376개의 case로, `"Survived = no"`에 해당하는 case는 189개, `"Survived = yes"`에 해당하는 case는 187개이다. case의 행 번호는 `svm.model.rd$index`를 이용하여 확인할 수 있다.
